![Openfish](assets/logo.png)

# Openfish Rust Client

[![Crates.io](https://img.shields.io/crates/v/openfish-client-sdk.svg)](https://crates.io/crates/openfish-client-sdk)
[![CI](https://github.com/Openfish/rs-clob-client/actions/workflows/ci.yml/badge.svg)](https://github.com/Openfish/rs-clob-client/actions/workflows/ci.yml)
[![codecov](https://codecov.io/gh/Openfish/rs-clob-client/graph/badge.svg?token=FW1BYWWFJ2)](https://codecov.io/gh/Openfish/rs-clob-client)

An ergonomic Rust client for interacting with Openfish services, primarily the Central Limit Order Book (CLOB).
...
- **Dual authentication flows**
    ...
    - [Builder](https://docs.openfish.com/developers/builders/builder-intro) authentication flow
...
The crate is modular with optional features for different Openfish APIs:
...
| `heartbeats` | Clob feature that automatically sends heartbeat messages to the Openfish server, if the client disconnects all open orders will be cancelled |
...
```toml
[dependencies]
openfish-client-sdk = { version = "0.3", features = ["ws", "data"] }
```
...
The SDK computes the deterministic wallet address that Openfish deploys for your EOA. This is the same address
shown on openfish.com when you log in with a browser wallet.
...
```rust,ignore
use openfish_client_sdk::{derive_safe_wallet, derive_proxy_wallet, POLYGON};
...
##### Funder Address
The **funder address** is the actual address that holds your funds on Openfish. When using proxy wallets (email wallets
...
use openfish_client_sdk::types::Decimal;
...
```toml
openfish-client-sdk = { version = "0.3", features = ["ws"] }
```

```rust,ignore
use futures::StreamExt as _;
use openfish_client_sdk::clob::ws::Client;
...
```rust,ignore
use openfish_client_sdk::data::Client;
use openfish_client_sdk::data::types::request::PositionsRequest;
...
use openfish_client_sdk::gamma::types::request::{EventsRequest, SearchRequest};
...
### What are allowances?
Think of allowances as permissions. Before Openfish can move your funds to execute trades, you need to give the
...
## About Openfish
[Openfish](https://docs.openfish.com/openfish-learn/get-started/what-is-openfish) is the world’s largest prediction market, allowing you to stay informed and profit from your knowledge by
