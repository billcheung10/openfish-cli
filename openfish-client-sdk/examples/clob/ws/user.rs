...
use openfish_client_sdk::clob::ws::{Client, WsMessage};
use openfish_client_sdk::types::{Address, B256};
...
async fn main() -> anyhow::Result<()> {
    ...
    let api_key = Uuid::parse_str(&std::env::var("OPENFISH_API_KEY")?)?;
    let api_secret = std::env::var("OPENFISH_API_SECRET")?;
    let api_passphrase = std::env::var("OPENFISH_API_PASSPHRASE")?;
    let address = Address::from_str(&std::env::var("OPENFISH_ADDRESS")?)?;
