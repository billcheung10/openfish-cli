...
//! ```
//!
//! Requires `OPENFISH_PRIVATE_KEY` environment variable to be set.
...
use openfish_client_sdk::clob::types::request::{
...
use openfish_client_sdk::types::{Decimal, U256};
...
async fn main() -> anyhow::Result<()> {
    ...
    let private_key = std::env::var(PRIVATE_KEY_VAR).expect("Need OPENFISH_PRIVATE_KEY");
    let signer = LocalSigner::from_str(&private_key)?.with_chain_id(Some(POLYGON));

    let config = Config::builder().use_server_time(true).build();
    let client = Client::new("https://clob.openfish.com", config)?
