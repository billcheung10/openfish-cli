...
/// RTDS (Real-Time Data Socket) client for streaming Openfish data.
///
...
///
/// ```rust, no_run
/// use openfish_client_sdk::rtds::Client;
...
impl Default for Client<Unauthenticated> {
    fn default() -> Self {
        Self::new("wss://ws-live-data.openfish.com", Config::default())
...
impl<S: State> Client<S> {
    ...
    /// ```no_run
    /// use openfish_client_sdk::rtds::Client;
    /// use openfish_client_sdk::ws::config::Config;
    /// use futures::StreamExt;
    /// use tokio::pin;
    ///
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// let client = Client::new("wss://rtds.openfish.com", Config::default())?;
