...
/// Generic set of credentials used to authenticate to the Openfish API. These credentials are
...
pub mod state {
    ...
    pub struct Authenticated<K: Kind> {
        ...
        /// passed in the L2 headers ([`super::HeaderMap`]) `OPENFISH_SIGNATURE` field.
...
pub(crate) mod l1 {
    ...
    pub(crate) const OPENFISH_ADDRESS: &str = "OPENFISH_ADDRESS";
    pub(crate) const OPENFISH_NONCE: &str = "OPENFISH_NONCE";
    pub(crate) const OPENFISH_SIGNATURE: &str = "OPENFISH_SIGNATURE";
    pub(crate) const OPENFISH_TIMESTAMP: &str = "OPENFISH_TIMESTAMP";
    ...
    pub(crate) async fn create_headers<S: Signer>(
        ...
        map.insert(OPENFISH_NONCE, naive_nonce.to_string().parse()?);
        map.insert(OPENFISH_SIGNATURE, signature.to_string().parse()?);
        map.insert(OPENFISH_TIMESTAMP, timestamp.to_string().parse()?);
...
pub(crate) mod l2 {
    ...
    pub(crate) const OPENFISH_ADDRESS: &str = "OPENFISH_ADDRESS";
    pub(crate) const OPENFISH_API_KEY: &str = "OPENFISH_API_KEY";
    pub(crate) const OPENFISH_PASSPHRASE: &str = "OPENFISH_PASSPHRASE";
    pub(crate) const OPENFISH_SIGNATURE: &str = "OPENFISH_SIGNATURE";
    pub(crate) const OPENFISH_TIMESTAMP: &str = "OPENFISH_TIMESTAMP";
    ...
    pub(crate) async fn create_headers<K: Kind>(
        ...
        map.insert(OPENFISH_API_KEY, state.credentials.key.to_string().parse()?);
        map.insert(
            OPENFISH_PASSPHRASE,
            state.credentials.passphrase.expose_secret().parse()?,
        );
        map.insert(OPENFISH_SIGNATURE, signature.parse()?);
        map.insert(OPENFISH_TIMESTAMP, timestamp.to_string().parse()?);
...
pub mod builder {
    ...
    pub(crate) const OPENFISH_BUILDER_API_KEY: &str = "OPENFISH_BUILDER_API_KEY";
    pub(crate) const OPENFISH_BUILDER_PASSPHRASE: &str = "OPENFISH_BUILDER_PASSPHRASE";
    pub(crate) const OPENFISH_BUILDER_SIGNATURE: &str = "OPENFISH_BUILDER_SIGNATURE";
    pub(crate) const OPENFISH_BUILDER_TIMESTAMP: &str = "OPENFISH_BUILDER_TIMESTAMP";

    #[derive(Clone, Debug, Deserialize, Serialize)]
    #[serde(rename_all = "UPPERCASE")]
    #[expect(
        clippy::struct_field_names,
        reason = "Have to prefix `openfish_builder` for serde"
    )]
    struct HeaderPayload {
        openfish_builder_api_key: String,
        openfish_builder_timestamp: String,
        openfish_builder_passphrase: String,
        openfish_builder_signature: String,
    }

    /// Configuration used to authenticate as a [Builder](https://docs.openfish.com/developers/builders/builder-intro). Can either be [`Config::local`]
            ...
            match &self.config {
                Config::Local(credentials) => {
                    ...
                    map.insert(
                        ...
                        credentials.key.to_string().parse()?,
                    );
                    map.insert(
                        OPENFISH_BUILDER_PASSPHRASE,
                        credentials.passphrase.expose_secret().parse()?,
                    );
                    map.insert(OPENFISH_BUILDER_SIGNATURE, signature.parse()?);
                    map.insert(OPENFISH_BUILDER_TIMESTAMP, timestamp.to_string().parse()?);
                ...
                Config::Remote { host, token } => {
                    ...
                    map.insert(
                        ...
                        remote_headers.openfish_builder_signature.parse()?,
                    );
                    map.insert(
                        OPENFISH_BUILDER_TIMESTAMP,
                        remote_headers.openfish_builder_timestamp.parse()?,
                    );
                    map.insert(
                        OPENFISH_BUILDER_API_KEY,
                        remote_headers.openfish_builder_api_key.parse()?,
                    );
                    map.insert(
                        OPENFISH_BUILDER_PASSPHRASE,
                        remote_headers.openfish_builder_passphrase.parse()?,
                    );
...
mod tests {
    ...
    async fn l1_headers_should_succeed() -> anyhow::Result<()> {
        ...
        assert_eq!(
            headers[l1::OPENFISH_ADDRESS],
            "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266"
        );
        assert_eq!(headers[l1::OPENFISH_NONCE], "23");
        assert_eq!(
            headers[l1::OPENFISH_SIGNATURE],
        ...
        assert_eq!(headers[l1::OPENFISH_TIMESTAMP], "10000000");
    ...
    async fn l2_headers_should_succeed() -> anyhow::Result<()> {
        ...
        assert_eq!(
            headers[l2::OPENFISH_ADDRESS],
            "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266"
        );
        assert_eq!(
            headers[l2::OPENFISH_PASSPHRASE],
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        );
        assert_eq!(headers[l2::OPENFISH_API_KEY], Uuid::nil().to_string());
        assert_eq!(
            headers[l2::OPENFISH_SIGNATURE],
            "eHaylCwqRSOa2LFD77Nt_SaTpbsxzN8eTEI3LryhEj4="
        );
        assert_eq!(headers[l2::OPENFISH_TIMESTAMP], "1");
    ...
    async fn builder_headers_should_succeed() -> Result<()> {
        ...
        assert_eq!(
            headers[builder::OPENFISH_BUILDER_API_KEY],
            Uuid::nil().to_string()
        );
        assert_eq!(
            headers[builder::OPENFISH_BUILDER_PASSPHRASE],
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        );
        assert_eq!(headers[builder::OPENFISH_BUILDER_TIMESTAMP], "1");
