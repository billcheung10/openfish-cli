...
pub const PRIVATE_KEY_VAR: &str = "OPENFISH_PRIVATE_KEY";
...
// Source: https://github.com/Openfish/builder-relayer-client
...
/// Init code hash for Openfish Proxy wallets (EIP-1167 minimal proxy)
...
pub struct WalletContractConfig {
    /// Factory contract for Openfish Proxy wallets (Magic/email wallets).
...
/// Derives the Openfish Proxy wallet address for an EOA using CREATE2.
///
/// This is the deterministic address of the EIP-1167 minimal proxy wallet
/// that Openfish deploys for Magic/email wallet users.
///
/// # Arguments
...
/// that Openfish deploys for browser wallet users.
///
/// # Arguments
/// * `eoa_address` - The externally owned account (EOA) address
/// * `chain_id` - The chain ID (e.g., 137 for Polygon mainnet)
