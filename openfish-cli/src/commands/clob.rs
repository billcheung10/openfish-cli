...
pub enum ClobCommand {
    ...
    MarketOrder {
        ...
        #[arg(long)]
        side: CliSide,
        /// Amount (FISH for buys, shares for sells)
        #[arg(long)]
        amount: String,
    ...
    CreateApiKey {
        /// Invitation code for first-time API key creation
        #[arg(long, env = "OPENFISH_INVITATION_CODE")]
        invitation_code: Option<String>,
        /// Write real API credentials to an agent env file with 0600 permissions
        #[arg(long)]
        agent_env_file: Option<PathBuf>,
        /// Skip the read-only /agent/account/state credential smoke test
        #[arg(long)]
        no_smoke_test: bool,
    },
...
pub async fn execute(
    ...
    let unauth = match std::env::var("OPENFISH_CLOB_HOST") {
        Ok(h) if !h.is_empty() => {
            let config = clob::Config::builder().geoblock_host(h.clone()).build();
            clob::Client::new(&h, config)?
        }
            ...
            if let Some(path) = agent_env_file.as_deref() {
                ...
                if !no_smoke_test {
                    let client = unauth
                        ...
                        .credentials(result.clone())
                        .authenticate()
                        .await?;
                    let _ = client.agent_account_state().await?;
                    eprintln!("Credential smoke test passed: GET /agent/account/state");
                }
            }
...
fn write_agent_env_file(
    ...
    credentials: &openfish_client_sdk::auth::Credentials,
) -> Result<()> {
    if let Some(parent) = path.parent().filter(|p| !p.as_os_str().is_empty()) {
        std::fs::create_dir_all(parent)?;
    }

    let host = std::env::var("OPENFISH_CLOB_HOST")
        .unwrap_or_else(|_| "https://api.openfish.me/".to_owned());
    let content = format!(
        "OPENFISH_CLOB_HOST={}\nOPENFISH_ADDRESS={:#x}\nOPENFISH_API_KEY={}\nOPENFISH_SECRET={}\nOPENFISH_PASSPHRASE={}\n",
        host,
        signer.address(),
